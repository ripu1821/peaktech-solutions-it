...
async function addEmployee(name, role) {
  try {
    const { error } = await database.from("peaktech-solutions-it").insert([{ name, role }]);
    if (error) {
      console.error("Error adding employee:", error.message);
      return null;
    }
    const { data, error: fetchError } = await database
      .from("peaktech-solutions-it")
      .select("*")
      .eq("name", name)
      .single();
...
async function deleteEmployee(id) {
  try {
    const { error } = await database.from("peaktech-solutions-it").delete().eq("id", id);
    if (error) {
